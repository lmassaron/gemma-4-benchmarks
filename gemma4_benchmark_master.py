...
def create_notebook(model_id, results):
    ...
    # Add text results
    cells.append(
        {"cell_type": "markdown", "metadata": {}, "source": ["## 1. Text Generation\n"]}
    )
    cells.append(
        {
            "cell_type": "code",
    ...
    cells.append(
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": ["## 2. Thinking Mode (Reasoning)\n"],
        }
    )
    ...
    cells.append(
        {
            ...
            "outputs": [
                {
                    "data": {"text/markdown": ["### Thinking Process"]},
                    "output_type": "display_data",
                },
                {
                    "data": {"text/markdown": [f"> {parsed['thinking']}"]},
                    "output_type": "display_data",
                },
                {
                    "data": {"text/markdown": ["### Final Answer"]},
                    "output_type": "display_data",
                },
                {
                    "data": {"text/markdown": [f"{parsed['answer']}"]},
                    "output_type": "display_data",
                },
            ],
            ...
            "outputs": [
                {
                    "data": {
                        "text/markdown": [
                            f"### Single Image Description\n{results['answers'].get('vision_single', 'N/A')}"
                        ]
                    },
                    "output_type": "display_data",
                }
            ...
            "outputs": [
                {
                    "data": {
                        "text/markdown": [
                            f"### Visual Logic (Cow Test)\n{results['answers'].get('vision_cow', 'N/A')}"
                        ]
                    },
                    "output_type": "display_data",
                }
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        targets = (
            ...
            else [m for m in MODELS if sys.argv[1] in m]
        )
        if not targets:
